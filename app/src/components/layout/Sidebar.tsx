...
import { LayoutDashboard, Users, ClipboardList, CreditCard, Settings, ChevronDown, ChevronRight, Home, Car, Ticket, UtensilsCrossed, LogOut, Menu, X, Tags, Calendar, User, MessageSquare, Users2, Calculator, PieChart, Activity, Gift, Megaphone, Crown, Store } from 'lucide-react';
...
const menuItems: MenuItem[] = [
  ...
  { 
    ...
    children: [
      ...
      { key: 'costs', label: '成本管理', icon: Calculator, path: 'costs' },
      { key: 'cost-stats', label: '成本统计', icon: PieChart, path: 'cost-stats' },
    ]
  },
  ...
  { 
    ...
    icon: Gift, 
    children: [
      { key: 'coupons', label: '优惠券管理', icon: Gift, path: 'coupons' },
  ...
  { 
    ...
    icon: Crown, 
    children: [
      { key: 'member-levels', label: '会员等级', icon: Crown, path: 'member-levels' },
    ]
  },
  { key: 'merchants', label: '商家管理', icon: Store, path: 'merchants' },
  { key: 'usage-monitor', label: '额度监控', icon: Activity, path: 'usage-monitor' },
...
export default function Sidebar({ collapsed, onToggle, currentPath, onNavigate, onLogout }: SidebarProps) {
  const [expandedMenus, setExpandedMenus] = useState<string[]>(['orders', 'configs', 'car-management', 'staff-management', 'marketing', 'membership']);
